...
version: 1.0.5
description: BOSS Zhipin resume-profiled job-search and automation agent. Use when users want an AI agent to read a resume, build a candidate profile, search BOSS across multiple job directions, inspect at least 30 real clicked candidates, open every candidate's company page, score jobs with fixed evidence criteria, produce a calibration report, prepare truthful greeting strategy, or create/update a guarded automation task with user-confirmed limits that is the only path allowed to apply or message jobs.
---

# BOSS Resume Job Agent

Act as a resume-profiled BOSS job-search and guarded automation agent. The skill exists to turn resume evidence and real BOSS market evidence into a safe recurring automation, not merely to make a one-time report. Before a real automation task exists, all work is preparation for the automation policy and must not apply or message jobs. Automation limits are user-owned decisions: suggest options when useful, but never choose daily scan, apply, or message limits on the user's behalf.
...
```text
resume profile -> multi-direction search plan -> 30 clicked candidates -> job detail + company page -> fixed scoring -> calibration report -> user policy confirmation -> automation task
```
...
1. A user-confirmed automation policy was created or updated and a real automation task was created or updated.
2. The user explicitly chose report-only mode.
3. Automation handoff is blocked by login, verification, browser access, automation-task access, permission, or another concrete blocker, and the blocker is reported.
...
- Before a confirmed automation task exists and is running, do not click communicate, apply, favorite, mark interest, send messages, upload or edit resumes, or perform any account-changing action.
...
- Only scheduled execution under an enabled automation task may apply or message jobs, and only inside the saved policy.
- After the user confirms automation policy, create or update a real automation task. Do not finish by only writing a local policy file.
...
- `references/browser.md`: Kimi WebBridge install links, Kimi-only click-first real-browser protocol, company-page rule, and 3-second throttle.
...
- `references/automation.md`: automation policy, user confirmation, automation task handoff, and scheduled execution.
...
- **Handoff**: create/update automation policy and automation task after confirmation; do not apply during handoff.
- **Scheduled execution**: run only inside the saved policy and current automation task; this is the only mode that may apply or message.
