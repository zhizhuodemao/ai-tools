...
适合不支持 Codex Plugin、但支持 Skill 的 Agent。源码在：
...
`skill/boss-resume-job-agent` 是通用 Skill 源，`plugin/boss-resume-agent-plugin/skills/boss-resume-job-agent` 是 Codex Plugin 内嵌副本。

更新 BOSS 能力时：

1. 先更新 `skill/boss-resume-job-agent`。
2. 同步到 `plugin/boss-resume-agent-plugin/skills/boss-resume-job-agent`。
3. 更新 `plugin/boss-resume-agent-plugin/.codex-plugin/plugin.json` 的版本号。
4. 确认两份 Skill 内容一致。
